...
function cvrpplot(cvrp, solution, title="")
    ...
    xmin = minimum(cvrp.nodes[:,1])
    xmax = maximum(cvrp.nodes[:,1])
    ymin = minimum(cvrp.nodes[:,2])
    ymax = maximum(cvrp.nodes[:,2])
    ...
    for i = 2:(n + 1)
        fig = scatter!([cvrp.nodes[i,1]], [cvrp.nodes[i,2]], color = "red", mark = :o, markersize = 3)
    end
    
    # Plota o depósito
    fig = scatter!([cvrp.nodes[1,1]], [cvrp.nodes[1,2]], color = "blue", mark = :square, markersize = 5, label = "Depósito")
    
    # Plota as rotas dos veículos
    colors = distinguishable_colors(length(solution))
    for (idx, route) in enumerate(solution)
        for i in 1:(length(route) - 1)
            fig = plot!([cvrp.nodes[route[i],1], cvrp.nodes[route[i+1],1]], 
                        [cvrp.nodes[route[i],2], cvrp.nodes[route[i+1],2]], 
                        color = colors[idx], lw = 1.5)
        end
        # Conectar o último ponto ao depósito
        fig = plot!([cvrp.nodes[route[end],1], cvrp.nodes[1,1]], 
                    [cvrp.nodes[route[end],2], cvrp.nodes[1,2]], 
                    color = colors[idx], lw = 1.5)
    end
